...
export default function AdminOutfitForm() {
  ...
  const handleThumbnailChange = (e) => {
    ...
    if (file) {
      ...
      // Clear specific thumbnail validation error if resolved
  ...
  const handleGalleryChange = (e) => {
    ...
    if (files.length > 0) {
      ...
      // Clear specific images validation error if resolved
      if (files.length >= 2) {
  ...
  const handleSubmit = async (e) => {
    ...
    // Media checks
    if (!thumbnail) {
      errors.thumbnail = 'A product thumbnail image is required.';
    }
    if (galleryImages.length < 2) {
      errors.images = 'Please select at least 2 gallery images (front & back view).';
    }
    ...
    formData.append('occasions', occasions.join(',')); // comma-separated strings as expected by parser
    ...
    if (tags) formData.append('tags', tags); // comma-separated string
    ...
    if (qualityNotes) formData.append('qualityNotes', qualityNotes);
    ...
    try {
      ...
      // Successfully created! Navigate back to admin dashboard catalog list
      navigate('/admin/outfits');
    } catch (err) {
      console.error('Error submitting outfit form:', err);
      // Retrieve Zod validation failures or standard error message
      if (err.response && err.response.data) {
        const errorData = err.response.data;
        if (errorData.errors && typeof errorData.errors === 'object') {
          // Flatten nested validation errors from Zod if available
          setValidationErrors(errorData.errors);
          setErrorMessage('Validation checks failed. Please inspect input values highlighted below.');
        } else {
          setErrorMessage(errorData.message || 'Server rejected request details.');
        }
      } else {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in relative pb-12">
      ...
      {/* Main Form container */}
      <form onSubmit={handleSubmit} className="space-y-8 text-xs text-brand-espresso">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div id="slug" className="space-y-1">
              ...
              <input
                ...
                placeholder="e.g. elegant-red-banarasi-lehenga"
                className={`w-full p-2.5 border rounded-soft bg-brand-cream/10 focus:outline-none focus:border-brand-espresso transition-colors ${
                  validationErrors.slug ? 'border-rose-400 focus:border-rose-500' : 'border-brand-dust/20'
                }`}
              />
              <span className="text-[9px] text-brand-dust leading-normal block">
                Lowercase words separated by dashes. Autogenerated by backend if left blank.
              </span>
              {validationErrors.slug && (
                <span className="text-[10px] text-rose-600 block">{validationErrors.slug}</span>
              )}
            </div>
        ...
        <section className="bg-white rounded-soft border border-brand-dust/10 p-6 space-y-6 shadow-sm">
          <h3 className="font-serif text-base font-semibold border-b border-brand-cream pb-2">
            4. Image Assets Upload (Cloudinary CDN Pipeline)
          </h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div id="thumbnail" className="space-y-2">
              <label className="block uppercase tracking-wider font-semibold text-brand-dust">
                Outfit Thumbnail * (Square aspect crop recommended)
              </label>
              <div className="relative border-2 border-dashed border-brand-dust/25 hover:border-brand-espresso/50 rounded-luxury p-5 text-center transition-colors">
                ...
                <p className="text-[11px] text-brand-dust mt-1 font-light">
                  {thumbnail ? thumbnail.name : 'Click to select or drag thumbnail file'}
                </p>
              </div>

              {thumbnailPreview && (
                <div className="relative w-20 h-20 rounded-md overflow-hidden bg-brand-cream border border-brand-dust/10">
            ...
            <div id="images" className="space-y-2">
              <label className="block uppercase tracking-wider font-semibold text-brand-dust">
                Product Gallery Images * (Upload at least 2 files)
              </label>
              <div className="relative border-2 border-dashed border-brand-dust/25 hover:border-brand-espresso/50 rounded-luxury p-5 text-center transition-colors">
                <input
                  type="file"
                ...
                <p className="text-[11px] text-brand-dust mt-1 font-light">
                  {galleryImages.length > 0
                    ...
                    : 'Click to select or drag at least 2 images'}
                </p>
              </div>
              ...
              {galleryPreviews.length > 0 && (
                <div className="flex gap-2 flex-wrap pt-1">
                  {galleryPreviews.map((src, index) => (
                    <div key={index} className="w-12 h-12 rounded-md overflow-hidden bg-brand-cream border border-brand-dust/10">
                      <img src={src} alt={`Gallery Preview ${index + 1}`} className="w-full h-full object-cover" />
                    </div>
                  ))}
                </div>
              )}
        ...
        <div className="flex items-center justify-end gap-4 border-t border-brand-cream pt-6">
          ...
          >
            {isSubmitting ? 'Uploading Details...' : 'Save & Publish Outfit'}
          </button>
        </div>
      </form>
      ...
      {isSubmitting && (
        <div className="fixed inset-0 bg-brand-espresso/45 z-[999] flex items-center justify-center flex-col space-y-3">
          ...
          <span className="text-white text-xs uppercase tracking-widest font-bold">Uploading files to Cloudinary CDN...</span>
          <span className="text-brand-cream/80 text-[10px]">This may take a moment. Please do not close this tab.</span>
        </div>
      )}
    </div>
  );
